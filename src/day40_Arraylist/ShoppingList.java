    ...
    public static void main(String[] args) {
        ...
        if (input.nextLine().equals("yes")) {
            ...
            if (Character.isDigit(removeItem.charAt(0))) { // checks the first character of remove item, checks if it is a number
               int number = Integer.parseInt(removeItem);// converts the String to int
               shoppingList.remove(number - 1);// converts the number to an index. if the user wants to remove first item, they would give 1, so to remove the first index 1-1=0
            } else {
